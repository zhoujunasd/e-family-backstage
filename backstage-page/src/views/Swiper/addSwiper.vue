<template>
    <div class="swiper-wrap">
        <el-card v-loading="loading">
            ...
            <el-form :model="formData" ref='form' status-icon  :rules="rules" size='mini' label-width="120px" label-position="left">
        ...
        data(){
            ...
            return{
                ...
                rules:{
                    ...
                    newsId:[{ required: true, message: '请轮播图对应新闻！', trigger: 'blur'}],
                    img:[{ type: 'string', message: '请选择轮播图头图！', trigger: 'blur'}],
                    sort:[
                    ...
                    status:[{ type: 'number', message: '类型判断是数字！', trigger: 'blur',},],
                }
            }
        },
        methods:{
            EdittForm(form){
                this.$refs[form].validate(valid => {
                    if (valid) {
                    this.$axios.patch(`/swiper/updateSwiper`,this.formData).then(res => {
                        // console.log(res);
                        if(res.code == 200){
                            this.$message({
                                message: res.msg,
            ...
            getData(){
            this.loading = true
            this.$axios.get(`/swiper/getswiper/${this.$route.params.id}`).then(res => {
            // console.log(res);
            if(res.code == 200){
                this.loading = false
